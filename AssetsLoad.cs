            ...
            foreach (var _ in 程序集.GetManifestResourceNames())
            {
                ...
                if (_.StartsWith(标识))         // 仅处理特定命名空间+目录下的资源
                {
                    ...
                    if (下标 != -1)
                    {
                        ...
                        相对 = 路径 + 文件后缀名;
                        Stationeers_Fixes_Package_Initialization.BepinExTool().BepinExMessage(BepInEx.Logging.LogLevel.Warning, $"已还原文件路径: {相对}");
                    ...
                    else
                    {
                        ...
                        if (_.IndexOf(Font路径) != -1)
                        {
                            using (Stream 读 = 程序集.GetManifestResourceStream(_))
                            {
                                ...
                                Stationeers_Fixes_Package_Initialization.BepinExTool().BepinExMessage(BepInEx.Logging.LogLevel.Warning, $"成功加载Font字体: {Font字体.name}");
                            }
                        }
        ...
        private List<UnityEngine.Object> 查找所有资源()
        {
            ...
            foreach (var obj in All)
            {
                ...
                else if (type == typeof(TMP_FontAsset))
                {
                    ...
                    {
                        _内置TMP字体 = 字体;
                        Stationeers_Fixes_Package_Initialization.BepinExTool().BepinExMessage(BepInEx.Logging.LogLevel.Warning, $"{Plugin_Config.Name}.成功加载TMP字体: {_内置TMP字体.name}");
                    }
                }
            }
            return 有效;
