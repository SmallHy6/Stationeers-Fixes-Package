            ...
            switch (逻辑写入开关控件.Action)
            {
                case InteractableType.Button1:
                    return new 链接选择面板渲染分支选择消息.消息结构
                    {
                        ...
                        可链接物渲染分支消息 = new 链接选择面板渲染分支选择消息.可链接物渲染分支消息 { 空数据网么 = (逻辑写入开关.OutputNetwork1DevicesSorted == null || 逻辑写入开关.OutputNetwork1DevicesSorted.Count <= 1) ? true : false, 可链接物体表 = 逻辑写入开关.OutputNetwork1DevicesSorted?.Where(d => d != (ILogicable)逻辑写入开关 && d.IsLogicWritable()) }
                    };
